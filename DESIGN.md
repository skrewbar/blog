...
- **Post meta:** On `PostCard` and the post page header, below the title use one meta row (`PostMeta`): `date | reading time | category`, each item prefixed with a Lucide icon (`Calendar`, `Clock`, `Folder`, `size-3.5`, `aria-hidden`) and separated by `|` (`text-border`). Row color is `text-muted-foreground`; the category is a link with `hover:text-brand`.
- **Cards (`PostCard`, `CategoryPreviewCard`):** Card hover uses `bg-brand-wash`. Titles (post title / category name) go `text-brand-500` (light) / `text-brand-300` (dark) + underline via `group-hover/card`. On `PostCard`, meta (date, reading time) and description also use `text-brand-500` in light / `text-brand-400` in dark. No lift or scale. Nested category/tag/post links remain pointer-events-auto where needed.
