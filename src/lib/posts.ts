...
export function getPublishedPosts(): Post[] {
  return sortPostsByDate(allPosts.filter((post) => !post.draft || includeDrafts))
}
...
export function getPostsByTag(tag: string): Post[] {
  return getPublishedPosts().filter((post) => post.tags.some((item) => item.toLowerCase() === tag.toLowerCase()))
}

export function getAllCategories(): string[] {
  return [...new Set(getPublishedPosts().map((post) => post.category))].sort((a, b) => a.localeCompare(b))
}
